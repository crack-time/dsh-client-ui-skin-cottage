...
export function createCottageCardStore() {
    let state = { ...COTTAGE_DEFAULTS, loaded: false };
...
export function CottageSettingsCard(props) {
    ...
    const title = t('title');
    return (_jsxs("li", { "data-cottage-settings": true, className: cn('cottage-settings-card', open && 'cottage-settings-card-open'), children: [_jsxs("button", { type: "button", className: "cottage-settings-header", "aria-expanded": open, "aria-label": `${t(open ? 'collapse' : 'expand')}: ${title}`, onClick: () => setOpen(!open), children: [_jsxs("span", { className: "cottage-settings-headText", children: [_jsx("span", { className: "cottage-settings-name", children: title }), _jsx("span", { className: "cottage-settings-description", children: t('description') })] }), dirty ? _jsx("span", { className: "cottage-settings-pending", children: t('unsaved') }) : null, _jsx(IconChevronDownOutline14, { className: cn('cottage-settings-chevron', open && 'cottage-settings-chevron-open') })] }), open ? (_jsxs("div", { className: "cottage-settings-body", children: [_jsxs("label", { className: "cottage-settings-row", children: [_jsx("span", { className: "cottage-settings-label", children: t('wallpaperUrl') }), _jsx("input", { type: "text", value: wallpaper, placeholder: "https://\u2026", spellCheck: false, onChange: (e) => setWallpaper(e.target.value) }), _jsxs("span", { className: "cottage-settings-controls", children: [canPickLocally ? (_jsx("button", { type: "button", className: "cottage-settings-pick", disabled: busy, onClick: () => {
                                            void handlePick();
                                        }, children: t('pick') })) : (_jsx("span", { className: "cottage-settings-hint", children: t('unsupported') })), pickedName ? (_jsx("button", { type: "button", className: "cottage-settings-pick", disabled: busy, onClick: () => {
                                            void handleClearLocal();
                                        }, children: t('removeLocal') })) : null] }), pickedName ? (_jsxs("span", { className: "cottage-settings-hint", children: [t('picked'), pickedName] })) : null, _jsx("span", { className: "cottage-settings-hint", children: t('wallpaperUrlHint') })] }), _jsxs("div", { className: "cottage-settings-footer", children: [failed ? (_jsx("p", { className: "cottage-settings-failed", role: "status", children: t('saveFailed') })) : null, _jsx("button", { type: "button", className: "cottage-settings-discard", disabled: !dirty || saving, onClick: () => {
                                    setWallpaper(server);
                                    setFailed(false);
                                }, children: t('discard') }), _jsx("button", { type: "button", className: "cottage-settings-save", disabled: !dirty || saving, onClick: () => {
                                    void commit();
 ...
 */
export function installCottageSettingsCard(ctx, store) {
