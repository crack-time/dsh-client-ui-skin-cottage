...
function apply(ctx) {
	...
	function ensureArchiveButton() {
		...
		for (const btn of document.querySelectorAll("button[aria-label]")) {
			...
			if (labels.includes(label)) {
				...
				b.textContent = "📦";
				b.addEventListener("click", () => window.dispatchEvent(new CustomEvent("cottage:archive-toggle")));
	...
	try {
		ctx.slots.register({
			...
			id: "cottage-archive",
			order: 9999,
			registrant: "ui-skin-cottage: archive panel"
		}, ArchivePanel);
	} catch {}
