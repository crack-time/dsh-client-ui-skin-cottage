...
async function listLevel(sessionId, rel, query, signal) {
	...
	const res = await fetch("/plugins/@crack/dsh-client-ui-skin-cottage/api/mention/files?" + params.toString(), { signal });
	if (!res.ok) return {
		cwd: null,
		dir: rel,
		entries: [],
...
const COTTAGE_CONFIG_URL = "/plugins/@crack/dsh-client-ui-skin-cottage/api/config";
